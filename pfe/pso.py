...
def evaluate(position, col_idx, env):
    ...
    total_cost = env.avg_node_energy(power=True) #sum(task.exe_energy for task in tasks)
    success_rate = SuccessRate().eval(env.logger)
    ...
    score = (norm_success * 1) + (norm_latency * 1e1) + (norm_energy * 1e4)
    return score
...
class Particle:
    ...
    def __init__(self, flag, data_np, col_idx):
        ...
        self.env = Env(self.scenario, config_file="core/configs/env_config_null.json")
...
def run_pso(
    ...
    env_eval = Env(scenario_eval, config_file="core/configs/env_config_null.json")
    ...
    with contextlib.nullcontext() as executor:
...
def final_evaluate(position, scenario_config, flag):
    scenario = Scenario(config_file=scenario_config, flag=flag)
    env = Env(scenario, config_file="core/configs/env_config_null.json")
    ...
    config = {
        "env": {
            "dataset": "Pakistan",
            "flag": flag,  # Can change to Tuple50K or Tuple100K if desired.
            "refresh_rate": 0.001
        },
        "policy": "pso",
    }
...
if __name__ == "__main__":
    ...
    final_evaluate(solution[:], CONFIG_PATH, FLAG)
